...
| `maintenance` | Baca/simpan mode maintenance global. `value` boolean = override manual (paksa ON/OFF); `value:"auto"` = hapus override agar ikut jadwal; `schedule:{enabled,start,end,days}` = simpan **Jadwal Otomatis Harian** (format `HH:MM`, zona WIB, bawaan `09:00`-`17:00`; `days` 0=Min … 6=Sab = **hari aktif**, ON sepanjang hari; hari tidak dicentang mengikuti rentang jam; mendukung rentang lintas tengah malam). Respons memuat `maintenance` (efektif), `manual` (`null`/`true`/`false`), dan `schedule`. Dipakai submenu **Ubah password/Maintenance**. |
