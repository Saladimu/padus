...
- **Mode Maintenance**: sakelar **Mode Maintenance** di submenu **Ganti Kata Sandi Admin**. Saat ON, muncul jendela kecil merah berkedip "We're Getting Things Ready" dan input Student ID/PIN dinonaktifkan. Status disimpan di backend (global untuk semua perangkat). Perangkat yang sudah membuka halaman tetap dicek ulang: saat siswa mengetuk/mengisi kolom identitas, status maintenance diambil ulang dari server (throttle 60 detik) dan form langsung dikunci bila baru diaktifkan. Backend juga menolak `verify`/`submit` dengan `{ success:false, maintenance:true }` selama mode ON, sehingga halaman lama tidak bisa menulis data.
...
- **Ikon Pengaturan tersembunyi**: `#btnSettings` disembunyikan sampai logo tengah diketuk 5 kali (`toggleSettingsGear()`). Ketuk 5 kali lagi untuk menyembunyikan ikon dan menutup modal pengaturan bila sedang terbuka. Mode maintenance dipindah ke sakelar di submenu Ganti Kata Sandi Admin (bukan lagi 5x klik logo).
