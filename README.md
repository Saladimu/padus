...
- **Perataan cetak/PDF**: kolom **Tanggal** pada cetak **Riwayat Absensi Siswa** (`printHistoryRows` + header di `index.html`) dan kolom **Nama** pada cetak **Daftar Siswa** (`printStudents` + header di `index.html`) kini rata kiri (`text-align:left`) agar teks panjang lebih mudah dibaca; kolom lain tetap rata tengah.
