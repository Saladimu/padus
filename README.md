...
- **Peek Laporan Absensi Hari Ini**: klik/mengetuk tanggal di header (di bawah judul "ABSENSI PADUAN SUARA") membuka modal ringkas berisi daftar siswa yang sudah tercatat hari ini dengan **No**, **Nama**, **jam log-in** (waktu submit, format HH:MM), tombol **Riwayat** per siswa (`showPeekStudentHistory()`), plus jumlah siswa tercatat. Siswa berstatus **Izin** ditandai teks **"(Izin)"** berwarna oranye di samping namanya. Data diambil dari action `report` untuk tanggal hari ini dan mengikuti rentang tahun ekskul (di luar rentang akan ditampilkan peringatan).
