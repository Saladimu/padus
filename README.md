...
`sw.js` meng-cache aset statis (CSS, JS, `Absensi.md`, ikon, logo, halaman utama) agar aplikasi terbuka cepat pada kunjungan berikutnya dan tetap bisa diakses saat offline. Strategi: **stale-while-revalidate** untuk navigasi halaman (HTML dari cache ditampilkan segera, versi terbaru diunduh di latar belakang; fallback cache saat offline), **cache-first** untuk aset statis same-origin, dan **stale-while-revalidate** untuk font Google lintas-origin. Cache diberi versi (`choir-absensi-v26`); versi lama otomatis dibersihkan saat aktivasi, varian aset `app.js`/`styles.css` yang sudah tidak dipakai ikut dihapus (cache tetap ramping), dan jumlah entri dibatasi (100).
...
- **Re-check maintenance saat interaksi**: `refreshMaintenance(force)` mengambil ulang status dari server (di-throttle 10 detik untuk interaksi) dan dipanggil saat kolom `#studentIdentity` difokus/diklik (`guardMaintenanceInteraction()`) serta sebagai pemeriksaan tambahan saat submit `verify`/`submit`. Jika server ternyata ON, form langsung dikunci dan dikembalikan ke tahap 1 (`resetForm()`), tanpa perlu reload halaman. Backend `verifyStudent()`/`submitAttendance()` mengembalikan `{ success:false, maintenance:true }` sebagai lapis pengaman server (respons ini juga langsung mengunci form di sisi klien).
