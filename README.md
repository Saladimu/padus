...
- **Pengaturan Admin**: ikon roda gigi **tersembunyi**. Ketuk logo tengah **5 kali** untuk menampilkan atau menyembunyikan ikon (`toggleSettingsGear()`). Setelah ikon tampil, buka menu lalu masukkan kata sandi. Di dalamnya: ganti kata sandi, atur URL backend, tombol **ON/OFF** pada **Tahun ekskul padus** untuk mengaktifkan/mematikan batas rentang (MM-YYYY) yang membatasi laporan, dan **Test Koneksi** (action `ping`). Pengaturan otomatis **terkunci kembali 5 menit setelah dibuka** (sesi berbatas waktu tetap; aktivitas tidak memperpanjang). Menyembunyikan ikon (5 ketuk lagi) juga menutup modal pengaturan bila sedang terbuka.
...
- **Mode Maintenance**: sakelar **Mode Maintenance** di submenu **Ubah password/Maintenance**. Saat ON, muncul jendela kecil merah berkedip "We're Getting Things Ready" dan input Student ID/PIN dinonaktifkan. Status disimpan di backend (global untuk semua perangkat). Perangkat yang sudah membuka halaman tetap dicek ulang: saat siswa mengetuk/mengisi kolom identitas, status maintenance diambil ulang dari server (throttle 60 detik) dan form langsung dikunci bila baru diaktifkan. Backend juga menolak `verify`/`submit` dengan `{ success:false, maintenance:true }` selama mode ON, sehingga halaman lama tidak bisa menulis data.
...
- **Submenu Pengaturan dapat dilipat**: header **Koneksi Google Sheets**, **Backup Data**, dan **Ubah password/Maintenance** dapat dibuka/tutup (accordion) lewat `toggleSettingsSection()`; kondisinya otomatis tertutup saat pengaturan dikunci (`collapseSettingsSections()`) agar menu tetap rapi.
...
Ikon roda gigi di pojok kanan atas **tersembunyi**. Ketuk logo tengah **5 kali** untuk menampilkannya, lalu klik ikon itu untuk membuka **Pengaturan Admin**. Ketuk logo 5 kali lagi untuk menyembunyikan ikon (modal pengaturan ikut tertutup bila sedang terbuka):

- **Keamanan**: seluruh pengaturan dilindungi kata sandi (default `00000`, tidak ditampilkan di halaman web). Setelah terbuka, pengaturan otomatis **terkunci kembali 5 menit setelah dibuka** (sesi berbatas waktu tetap; interaksi apa pun tidak memperpanjang). **Salah kata sandi 3 kali** membuat form Buka Kunci **terkunci sementara selama 5 menit** (ditampilkan pesan "Terkunci sementara" beserta hitungan waktu; input dan tombol dinonaktifkan). Penghitung tersimpan di `localStorage` (kunci `choir_admin_lock_v1`) sehingga tidak bisa dilewati dengan memuat ulang halaman, dan otomatis direset setelah masa kunci berakhir atau saat kata sandi benar. Ganti kata sandi dilakukan lewat submenu **Ubah password/Maintenance**.
- **Ubah password/Maintenance**: submenu tersendiri di bagian paling bawah menu Pengaturan (tepat sebelum **Setup Backend**) dengan latar peringatan merah (alert). Berisi form kata sandi saat ini, kata sandi baru (minimal 4 karakter), dan konfirmasi. Submenu ini juga memuat sakelar **Mode Maintenance** untuk menyalakan/mematikan mode perawatan (disimpan di backend, berlaku global). Keduanya terkunci sampai kata sandi admin dimasukkan.
...
- **Submenu Ubah password/Maintenance**: form ganti kata sandi dipindah dari blok Keamanan ke submenu tersendiri di bagian paling bawah menu Pengaturan (sebelum **Setup Backend**) dengan latar peringatan merah (alert) dan header merah.
- **Ikon Pengaturan tersembunyi**: `#btnSettings` disembunyikan sampai logo tengah diketuk 5 kali (`toggleSettingsGear()`). Ketuk 5 kali lagi untuk menyembunyikan ikon dan menutup modal pengaturan bila sedang terbuka. Mode maintenance dipindah ke sakelar di submenu Ubah password/Maintenance (bukan lagi 5x klik logo).
- **Kunci otomatis Pengaturan**: pengaturan otomatis terkunci kembali **5 menit setelah dibuka** (sesi berbatas waktu tetap). Timer dimulai oleh `startSettingsLockTimer()` saat kata sandi benar dan **tidak di-reset oleh aktivitas**, sehingga pengaturan pasti terkunci meski sedang dipakai/meninjau laporan. Timer dihentikan `stopSettingsLockTimer()` saat terkunci manual.
...
- **Callout jumlah absensi di menu utama**: di bawah header menu utama ditampilkan banner gradasi oranye-merah beranimasi (`#mainCallout`, class `callout-banner`) berisi teks **"N siswa sudah absensi"**. Banner **disembunyikan bila belum ada siswa** yang absen hari ini, dan diperbarui dari data `report` peek (cache persisten saat halaman dibuka, prefetch senyap, saat peek dibuka, dan setelah submit absensi berhasil).
...
- **Sesi pengaturan berbatas waktu tetap**: pengaturan terkunci otomatis **5 menit setelah dibuka**, tanpa menghitung aktivitas. Sebelumnya penghitung diperpanjang oleh interaksi (`bindSettingsActivityReset()` pada `settingsModal`, `reportModal`, `studentModal`, `historyModal`, `adminModal` dengan `click`/`input`/`change`/`keydown`/`scroll`/`mousemove`/`touchstart`/`touchmove`), sehingga gerak mouse membuat auto-lock praktis tidak pernah terjadi. Kini semua binding aktivitas dihapus dan hanya `startSettingsLockTimer()` (dipanggil saat kata sandi benar) yang mengatur masa sesi 5 menit.
