...
- Setiap dokumen cetak (Laporan Absensi, Daftar Siswa, Riwayat Absensi) kini menyertakan **catatan kaki tahun ekskul** di pojok kanan bawah: `Tahun ekskul: <awal> s/d <akhir>` dengan format **Mmm-YY** (mis. `Aug-26`), memakai array bulan `MONTHS` (`monthYearToMmmYY()`). Catatan kaki diisi otomatis dari pengaturan Tahun ekskul saat print (`setPrintFootnotes()`) dan hanya muncul bila awal & akhir sudah diatur.
...
- **Catatan kaki tahun ekskul pada cetak/PDF**: elemen `#printFootnoteReport`, `#printFootnoteStudent`, dan `#printFootnoteHistory` di setiap area cetak menampilkan `Tahun ekskul: <Mmm-YY> s/d <Mmm-YY>` rata kanan berukuran 10px. Diisi `setPrintFootnotes()` tepat sebelum `window.print()` dari pengaturan Tahun ekskul.
